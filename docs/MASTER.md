...
**세 문서를 고치는 손이 지켜야 하는 형식이다.** 세션이 바뀌면 문맥이 초기화되므로 실제로는
여러 손이 같은 문서를 업서트한다(DECISIONS §9). 규약마다 강제자가 있고, 없으면 사유를 적는다.
2판은 파이어레인 · 하토르의 강제자를 옮겼다(DECISIONS §111).

```bash
python3 tools/check_docs.py      # 문서 ↔ 문서 — 문체 · 절 · PLAN 표 · 참조 · DECISIONS
python3 tools/doc_fsck.py        # 문서 ↔ 실물 — 경로 · 테스트 · 죽은 도구
python3 tools/check_counts.py    # 문서에 적힌 수 ↔ 실측
```

`doctor` 가 셋과 기획서 대조(`tools/docx_check.py`)를 부르고, 위반은 1 · 도구 고장은 2 로
끝나 둘을 가른다.
...
| 항목 | 규칙 |
|---|---|
| 인칭 | 3인칭. 화자는 개인이 아니라 저장소다 |
| 시제 | PLAN 은 미래 · MASTER 는 현재 · DECISIONS 는 과거. README 와 기획서(§0-7)는 시제 밖이다 |
| 종결 | 평어체 `-다`. 경어 · 명령형 · 청유형을 쓰지 않는다 |
| 어조 | 단정. 모르면 "미확인" 이라고 적는다 |
| 인용 | 폐기된 문언을 인용한 줄은 끝에 `<!--voice-ok-->` |

강제자 — `tools/check_docs.py::check_style` · `tools/check_docs.py::check_master`

### 0-2. 절과 참조

| 항목 | 규칙 |
|---|---|
| 절 번호 | 세 문서의 모든 `##` 에 번호가 있고, 겹치지 않고, 연속이다. 절은 지우지 않고 같은 시제의 다른 내용으로 채운다 |
| 문서명 | 다른 문서를 가리킬 때는 문서명을 앞에 적는다 — `MASTER §7-4` · `DECISIONS §109` · `PLAN #59` |
| PLAN 행 | `PLAN #N` 으로 적는다. 행은 §1 에만 살므로 절을 붙이지 않는다 |
| 남의 저장소 | 저장소 이름을 앞에 적는다 — `파이어레인 DECISIONS §205` · `하토르 D-0117` · `seshat PLAN #2`. 이름이 붙은 참조는 검사하지 않는다 |
| 실재 | 참조는 실재하는 절 · 행을 가리킨다. **DECISIONS 본문은 예외다** — 그때 있던 PLAN 행은 지금 없는 것이 정상이다 |

강제자 — `tools/check_docs.py::check_headings` · `tools/check_docs.py::check_refs`

★ **1판은 `PLAN §x #N` 꼴만 보았다.** `MASTER §x` · `DECISIONS §N` 은 아무도 보지 않았고,
코드와 픽스처가 지운 행(`#18` · `#19` · `#45` · `#50`)을 가리킨 채 남아 있었다.

### 0-3. PLAN

PLAN 머리의 「이 문서의 규약」 표가 정본이다 — §1 표 하나 · 제목의 행 수 · 영구 번호 ·
닫힌 상태 어휘.
...
| 항목 | 규칙 |
|---|---|
| 추가만 | 이미 적힌 절을 고치지 않는다. 그때 무엇을 몰랐는지가 사라진다 |
| 절의 모양 | `## §N. 제목` · 날짜 줄 `**YYYY-MM-DD**` · `### 배운 것` |
| 강제자 | §108 부터 `강제자 — ` 또는 `강제자 없음 — 사유` 줄이 있다. 옛 절에는 소급하지 않는다 |

강제자 — `tools/check_docs.py::check_decisions` · `tools/doctor.sh` 「DECISIONS 가 추가만 되었다」

★ **소급하지 않는 이유**는 하토르 D-0081 과 같다. 표기를 고치는 것은 판단을 바꾸지 않지만,
옛 절에 지금 강제자를 적으면 그때 하지 않은 판단을 사후에 지어낸다.

### 0-5. 실물

| 항목 | 규칙 |
|---|---|
| 경로 | README · MASTER · PLAN 과 DECISIONS 의 강제자 줄이 백틱으로 적은 경로는 실재한다. git 이 무시하는 경로(`infra/backend.hcl`)는 기계마다 있는 파일이라 뺀다 |
| 테스트 | `파일::이름` 은 그 파일에 그 함수가 있다 |
| 도구 | `tools/` 의 스크립트는 README · MASTER · 다른 도구 · 워크플로 중 어딘가에서 불린다 |

강제자 — `tools/doc_fsck.py::check_paths` · `tools/doc_fsck.py::check_tools`

### 0-6. 숫자

`<!--count:이름-->` 뒤의 정수는 주장이고 실측과 대조한다. 표시 없는 숫자는 서술이다.

강제자 — `tools/check_counts.py`
...
**정규식을 쓰는 검사는 카나리아를 든다.** 합성 문자열에서 기대한 것을 찾지 못하면 검사
자체가 2 로 끝난다. 0건이 목표인 검사는 깨끗해서 0 인지 죽어서 0 인지 가를 수 없다.

강제자 — `worker/tests/test_doc_enforcers.py`
...
프로젝트는 2026-07-18 에 선행 저장소로 시작했다. **Udemy 전용 도구였다** — 문항을 긁어 AWS
Translate 로 미리 번역해 두고 화면에서 사전을 조회했다(DECISIONS §1 · §5 · §7 이 그것을 인용한다).
사이트를 가리지 않는 확장으로 넓히기로 하면서 **GitHub 에서 CI/CD 를 갖춘 새 저장소로 다시
시작했다**(DECISIONS §117). 결정 기록은 2026-09-12 부터이고 첫 푸시는 2026-09-19 다.
...
후처리의 어미 변환을 코퍼스로 잰다(DECISIONS §107).
...
uv run --with kiwipiepy python tools/gen_endings.py        # 용언 목록을 고쳤을 때만
```

| 판정 | 뜻 |
|---|---|
| 맞음 | 정답과 같다. 겹치는 줄(`걸어요` — 걷다 · 걸다)은 다른 정답도 맞음 |
...
★ **글꼴은 브로커가 `FontFace` 로 올린다.** `chrome.runtime.getURL` 이 확장 주소를
준다. `content.css` 에 `@font-face` 를 두면 `insertCSS` 로 들어가 상대 경로가 **페이지
오리진**으로 풀리고 404 다 — 2026-09-16 까지 실사이트에서 한 번도 실리지 않았다
(DECISIONS §103). 페이지 CSP 가 막으면 콘솔에 `[st] 글꼴을 싣지 못했다` 가 찍힌다.

★ **SIL OFL 1.1 이다.** 사본을 `extension/fonts/LICENSE.txt` 에 함께 둔다 —
재배포 조건이고 웹스토어 심사에서도 묻는다.

★ **`web_accessible_resources` 에 등록한다.** 브로커가 `getURL` 로 만든
`chrome-extension://` 주소를 페이지가 읽으려면 공개돼 있어야 한다. **주입한 CSS 의
...
★ **이름표는 기다리는 동안에만 선다**(`::before` 의 `번역 중`). 다 된 번역에는 붙이지
않는다 — 글자가 한글이니 `한글` 은 덤이고, 원문과 번역을 가르는 일은 상자가 한다
(DECISIONS §106). DOM 이 아닌 이유는 브로커가 `textContent` 를 갈아 끼울 때 같이
지워지기 때문이다.
...
★ **기다리는 동안 뼈대를 세운다.** 이름표 `번역 중` 이 서고 본문 자리에
...
있고(DECISIONS §80), 그때 그 값은 그 엔진에만 들어간다.
...
| 워커 표시 | 로컬이 아니면 주황. 두 번째 방어선이다 — 확장이 픽스처에서는 팝업 설정과 상관없이 `127.0.0.1:8000` 을 때리고 토큰도 싣지 않는다(DECISIONS §106) |
...
| uv 캐시 | `uv.lock` 차집합 중 **캐시에 실제로 있는 것만**. 이웃과 겹치는 것은 남긴다(DECISIONS §118) |
