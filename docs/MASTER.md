...
★ **테두리와 그림자를 두르지 않는다.** 선을 두르면 상자가 하나 더 생기고
문항 아래에 상자가 둘 겹친다. 바탕색 한 겹과 왼쪽 2px 선만으로 번역임을
알린다 — **원문보다 눈에 먼저 들어오면 안 되는 자리다.**

★ **기다리는 상태가 클래스로 남는다**(`st-translation--loading`). 한 문항이
5초이고 그동안 표시가 없으면 눌렀는지조차 알 수 없다. 글자를 바꾸지 않고 빛이
한 번 지나가게 둔다 — 깜빡임보다 조용하다. `prefers-reduced-motion` 이면
움직이지 않고 색으로만 남는다.

★ **URL 은 칩으로 둔다.** 본문과 같은 흐름에 있으면 번역문처럼 읽힌다.
