...
### 7-0. 엔진 비교 (45유닛 · 배치 3 · 2026-09-14)
...
| 위반 종류 | term 4 | **term 2** |
| 환각 · 문체 | 0 · 0 | 0 · 0 |
| 재현성 | 유닛 이름까지 | 위반 유닛·종류까지 |

★ **첫 페이지 체감은 168초 대 5초다.** 로컬은 콜드 로딩이 붙는다.

★ **`bedrock` 쪽 위반이 더 적다.** `consumer` 와 `latency` 가 나오지 않고
`record` 2 건만 남는다.
...
★ **오버헤드는 요청 수에 비례한다.** 배치를 9 로 올리면 요청이 5회가 되어
오버헤드가 3분의 1이다. 다만 로컬 45유닛에서 배치 9 의 위반이 6 이었다(배치 3
은 3). **비용과 품질이 정면으로 맞바꿔지며** 그 판단이 PLAN §2-2 #45 다.
