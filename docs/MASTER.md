...
★ **상자를 그리지 않는다.** 바탕을 깔면 Udemy 보기 카드 안에 상자가 둘
겹친다 — 인용문 안의 인용문이다. 구분은 작은 이름표(`::before` 의 `한글`)와
글자 무게가 한다. **한국어라는 것 자체가 이미 가장 강한 구분 신호다.**

★ **색을 상속받는다. `prefers-color-scheme` 을 쓰지 않는다.** 그것은 OS
설정이지 이 페이지의 배경이 아니다 — 크롬이 어두운 설정이고 Udemy 가 밝은
테마이면 **흰 바탕에 흰 글씨**가 나온다(2026-09-16 에 실제로 났다). 부모의
`color` 를 물려받으면 사이트가 어느 테마든 저절로 맞는다.
...
★ **기다리는 상태는 이름표가 말한다**(`번역 중`). 한 문항이 5초이고 그동안
표시가 없으면 눌렀는지조차 알 수 없다. 상자가 없으니 흔들 것도 없다.
`prefers-reduced-motion` 이면 움직이지 않고 글자만 남는다.
