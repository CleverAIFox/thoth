...
**호스팅 실측은 문항 하나에 5.2초다**(45유닛 · 배치 3 · Nova Lite). 로컬은
같은 조건에서 80.7초이고 콜드 로딩 87.6초가 더 붙는다. 위반은 호스팅 2유닛 ·
로컬 3유닛이며, 로컬 값은 프롬프트를 고치기 전의 것이라 재측정 대기다.

로컬 실측은 GTX 1660 Ti 기준 문항 하나에 81초다(배치 3). 모델이 내려가 있으면 첫
...
- `bedrock` 은 같은 조건 두 판에서 어구가 갈린다. seed 가 Converse 공통 필드가
  아니라 `temperature 0` 만으로는 고정되지 않는다. 위반 유닛과 종류는 같다
...
- **프롬프트 오버헤드가 원문의 6.9배다.** 배치 3 에서 요청마다 프롬프트 머리
  1,149자가 다시 실린다. 배치를 키우면 줄지만 용어 위반이 는다
- 분석 경로(Firehose · Athena)는 아직 없다. 비용 실측이 거기 달려 있다
- UI 는 동작 수준이다. 한국어 길이 팽창은 실사이트 몇 곳에서만 봤다
- 확장의 기본 엔드포인트가 아직 로컬이다. 배포본을 쓰려면 팝업 고급에 넣는다
...
| | |
|---|---|
| 확장 · 워커 | 돈다. 확장 <!--count:ext_tests-->43건 · 워커 <!--count:worker_tests-->200건 |
